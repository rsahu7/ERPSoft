...
<c:url var="addAction" value="/add/Supplier"></c:url>
<c:set var="prevURL" value="${sessionScope.prevURL}" scope="session" />
	<form:form action="${addAction}" commandName="Supplier" method="POST" class="empform">
	<form:hidden path="Id" />
		<div id="left-table">
				<table>
					<tr>
						...
						<form:input path="code"  value="SUP${Supplier.code}"  readonly="true"/>
						</c:if>
						<c:if test="${empty Supplier.name}">
						<form:input path="code"  value=""  readonly="true"/>
						</c:if>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<fmt:formatDate var="creationdate" pattern="dd-MM-yyyy" value="${Supplier.creationdate}" />
		...
		<div id="savebuttons">
				<c:if test="${!empty Supplier.name}">
					<input id="save" type="submit" value="<spring:message text="Edit Supplier"/>" />
					<button type="button" onClick="if(!confirm('Are you sure you want to delete?')) return false; else javascript:location.href = '${contextroot}delete/Supplier/${Supplier.id}';" />Delete Supplier</button>
				</c:if>
				<c:if test="${empty Supplier.name}">
					<input id="save" type="submit" value="<spring:message text="Add Supplier"/>" />
				</c:if>
